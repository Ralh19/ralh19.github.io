  ...
  <section id="home"
    ...
    <Container>
      <div class="flex flex-col items-start justify-center w-full max-w-8xl mx-auto relative">
        <div class="absolute z-10 flex flex-col items-start w-full max-w-7xl left-1/2 top-1/2 
                 -translate-y-1/2 -translate-x-1/2" style="pointer-events: auto;">
          <!-- Halo -->
          <div class="absolute inset-0 z-0 pointer-events-none flex items-center justify-center">
            <div class="w-[900px] h-[400px] md:w-[1300px] md:h-[500px] rounded-full 
                     bg-highlight-color/25 dark:bg-highlight-color/30 blur-[100px] scale-110"></div>
          </div>
          ...
          <div class="relative z-10 w-full">
            ...
            <p class="text-xl md:text-2xl text-gray-600 dark:text-main-text-color-dark mb-10 
                     max-w-5xl">
              {{ hero.description }}
            </p>

            <div class="flex flex-col md:flex-row gap-4 md:gap-8 mb-10 items-stretch 
                     md:items-center w-full">
              <!-- Bouton "Hire Me" -->
              <button href="#contact" @click="handleScrollClick" class="text-2xl px-10 py-5 bg-highlight-color text-white rounded-2xl 
                       shadow-md hover:shadow-lg scale-animation hover:opacity-90 text-center transition-all cursor-pointer">
              ...
              <div ref="menuRef" class="relative inline-block text-left">
                <button @click.stop="toggleMenu" class="flex items-center justify-center gap-3 text-2xl md:px-10 py-5 w-full 
                         bg-highlight-color text-white rounded-2xl shadow-md hover:shadow-lg 
                         scale-animation hover:opacity-90 transition-all cursor-pointer">
                ...
                <transition enter-active-class="transition-all duration-300 ease-out"
                  ...
                  <div v-if="isMenuOpen" class="absolute left-0 mt-3 w-full bg-white dark:bg-main-gui-color-dark 
                           border border-gray-200 dark:border-main-background-color-dark 
                           rounded-2xl shadow-xl z-50 overflow-hidden backdrop-blur-sm">
                    <button @click="openPdf('fr')" class="w-full text-left px-6 py-4 text-gray-800 dark:text-main-text-color-dark 
                    ...
                    <button @click="openPdf('en')" class="w-full text-left px-6 py-4 text-gray-800 dark:text-main-text-color-dark 
                             hover:bg-highlight-color/10 dark:hover:bg-highlight-color/20 
                             flex items-center gap-3 text-lg transition-colors duration-200">
                      🇬🇧 <span>EN</span>
                    </button>
              ...
              <!-- Localisation -->
              <div class="flex items-center text-highlight-color mb-2 md:mb-0 justify-center 
                       md:justify-start max-sm:w-full">
                <i class="bi bi-geo-alt-fill text-3xl md:text-4xl bounce-animation"></i>
                <span class="text-2xl md:text-3xl ml-2 md:ml-3 dark:text-main-text-color-dark 
      ...
      <div class="hidden md:block absolute bottom-8 left-1/2 transform -translate-x-1/2 pb-6">
        <a href="#experience" @click="handleScrollClick" class="flex flex-col items-center text-2xl md:text-3xl text-gray-600 
      ...
      <div class="md:hidden absolute left-1/2 bottom-4 -translate-x-1/2 w-full flex justify-center 
               z-20 mt-10">
        <a href="#experience" @click="handleScrollClick" class="flex flex-col items-center text-2xl text-gray-600 
                 dark:text-main-text-color-dark hover:text-highlight-color 
                 dark:hover:text-highlight-color transition-colors">
